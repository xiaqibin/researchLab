...
***我们在之前的章节中证明的对于n>>p的情形下，n次实验成功的次数本身服从二项分布，可以近似的服从泊松分布，其中有假设每次实验独立且具有相同的成功概率，事实上，这个条件可以放宽，首先并不一定要求每次实验的成功概率相同，只需要n>>p即可，其次，每次实验不一定要独立，在n>>p的情况下几乎独立即可，也就是说。对于n次实验的成功次数X，如果n>>p，那么X近似服从于参数$$\lambda=\sum_{i}^{n}{p_i}$$的泊松分布***